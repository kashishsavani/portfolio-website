        ...
        <div className="d-flex inservices">
          ...
          <div className="sidebar">
            {services.map((service) => (
              <div
                key={service.id}
                className={`menu-item ${activeService === service.id ? "active" : ""}`}
                onClick={() => setActiveService(service.id)}
              >
                <i className={service.icon}></i> <span>{service.name}</span> <i className="fas fa-chevron-right"></i>
              </div>
            ))}
          </div>
          ...
          <div className="content">
            {services.map((service) => (
              <div key={service.id} className={`content-section ${activeService === service.id ? "active" : ""}`}>
                <h2 className="fw-bold d-flex justify-content-start fs-4">{service.name}</h2>
